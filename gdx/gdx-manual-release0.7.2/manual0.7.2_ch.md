...
- [Go DxChain 教程](#go-dxchain-%e6%95%99%e7%a8%8b)
  ...
  - [联络我们](#%e8%81%94%e7%bb%9c%e6%88%91%e4%bb%ac)
...
**<span style="color:red">注意: 当前您所下载的可执行文件仅支持64位系统</span>**
...
$ gdx
```
<span style="color:red">**注意：存储委托方和存储提供方都具有矿工挖矿的功能。**</span>
...
> miner.start()
```
<span style="color:red">**注意：为了方便您的使用体验，您用上方命令行生成的账号将不被密码保护。如果您希望创建一个拥有密码保护的账号，请在控制台中输入以下命令，系统将会提示您设置密码。**</span>
```js
> personal.newAccount()
```
...
### 步骤3.1 终止已经存在的节点
<span style="color:red">**注意：如果您之前从未开启过任何`gdx`程序，请跳过此步骤。**</span>
...
```
<span style="color:red">**注意：如果您之前创建账户时选择了创建密码，请您用`"YOUR_PASSWORD"`替换`""`。**</span>
...
<span style="color:red">**注意：在您执行*步骤3.7 委托方设定*之前请确保您已经和至少三个存储提供方连接，并且存储提供方的版本为*1.0.1*。您可以通过在控制台输入以下命令查看存储提供方的数量和版本。**
```shell
sclient.hosts
```
</span>
...
接下俩请您稍作等待，您的节点此时正在与存储提供方节点建立合约，你可以通过在控制台输入以下命令查看已经建立的合约数量，当合约数量为3的时候您可以进行下一步操作。
```js
> sclient.setconfig({})
```
...
$ ls "`pwd`/{$FILE_NAME}"
```
<span style="color:red">**注意：请把{$FILE_NAME}替换成您所要上传的文件名。**</span>

接下来，请您在控制台中输入以下命令。
```js
> sclient.upload("{$FILE_PATH}", "download.file")
```
<span style="color:red">**注意：请把{$FILE_PATH}替换成您刚刚取得的文件绝对路径。**</span>
...
<span style="color:red">**注意：如果您上传了您的私有文件而不是随机生成文件，请跳过这一步。**</span>
